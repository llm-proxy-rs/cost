...
pub fn render_index(base: &str, start: &str, end: &str, costs: &[CostByUser]) -> String {
    ...
    Page {
        ...
        info_rows: vec![
            InfoRow::raw(
                "Date Range",
                date_range_form(&make_path(base, "/users"), start, end),
            ),
...
pub fn render_detail(
    ...
    let costs = costs.to_vec();
    let empty = costs.is_empty();
    let display_name = user_email.unwrap_or(user_id);
    let total: f64 = costs.iter().map(|c| c.amount).sum();
    let currency = costs
        .first()
        .map(|c| c.currency.clone())
    ...
    Page {
        ...
        breadcrumbs: vec![
            Breadcrumb::link("Cost Explorer", make_path(base, "")),
            Breadcrumb::link("Users", make_path(base, "/users")),
        ...
        info_rows: vec![
            InfoRow::raw(
                "Date Range",
                date_range_form(&make_path(base, &format!("/users/{}", user_id)), start, end),
            ),
            InfoRow::new("User ID", user_id),
            InfoRow::new("Email", user_email.unwrap_or("unknown")),
            InfoRow::new("Total Cost", &format!("{:.2} {}", total, currency)),
        ],
        content,
        subpages: vec![],
...
mod tests {
    ...
    #[test]
    fn render_detail_unknown_email() {
        let html = render_detail("/", "2024-01-01", "2024-01-31", "abc-123", None, &[]);
