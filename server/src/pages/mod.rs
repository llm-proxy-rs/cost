...
pub fn sort_by_user(
    ...
    costs.sort_by(|a, b| {
        let cmp = match col {
            0 => {
                // Sort by display name, falling back to the id when no email is set.
...
pub fn sort_by_model(
    mut costs: Vec<CostByModel>,
    sort: Option<usize>,
    order: &str,
) -> Vec<CostByModel> {
    let Some(col) = sort else { return costs };
    ...
    costs.sort_by(|a, b| {
        let cmp = match col {
            0 => {
                // Sort by display name, falling back to the id when no name is set.
